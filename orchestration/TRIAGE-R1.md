...
| **R1-2** staleness floor was a flat 5 s, not `max(5 s, 3 × interval)` | should-fix | **Implemented, partly verified (correcting an earlier draft of this table):** `is_state_stale` takes an optional limit, `stale_threshold()` computes `max(5, 3 × interval_s)` from the limits cache, and `write_prereqs` calls `load_limits` first so the interval is available; `doctor`'s `state_fresh` uses the same limit. Verified: the CLI reads and caches the daemon's `config.interval_s` (a `bash -x` trace shows `cached_interval=1` on this machine, where the limit is 5 s either way) and the flat-floor refusal still behaves (6 s-old state → exit 5). **NOT verified:** the `interval_s ≥ 1.6 s` end-to-end case — the shipped fixture hardcodes `interval_s: 1` and the ad-hoc stub written for the check was malformed (the CLI rejected it, correctly, with "did not emit afanctl.status.v1 JSON"). A stub with a controllable `config.interval_s` is residual 3 below. |
